...
class TaskPrefixDataCollator(DataCollatorForSeq2Seq):
    def __call__(self, features, return_tensors=None):
        ...
        pred_features = features_df.loc[:, ~features_df.columns.isin(['aux_labels_1', 'expl_input_ids_1', 'expl_attention_mask_1'])].to_dict('records')
        expl_features_1 = features_df.loc[:, ~features_df.columns.isin(['labels', 'input_ids', 'attention_mask'])].rename(
...
class TaskPrefixTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss = self.alpha * pred_outputs.loss + (1. - self.alpha) * expl_outputs_1.loss
        return (loss, {'pred': pred_outputs, 'expl_1': expl_outputs_1}) if return_outputs else loss
    ...
    def prediction_step(
        ...
        if self.output_rationale:
            ...
            expl_outputs_1 = pred_outputs

        loss = self.alpha * pred_outputs[0]  + (1 - self.alpha) * expl_outputs_1[0]

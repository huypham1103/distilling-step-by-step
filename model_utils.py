...
class TaskPrefixDataCollator(DataCollatorForSeq2Seq):
    def __call__(self, features, return_tensors=None):
        ...
        pred_features = features_df.loc[:, ~features_df.columns.isin(['aux_labels_1', 'aux_labels_2', 'aux_labels_3', 'aux_labels_4','expl_input_ids_1', 'expl_attention_mask_1', 'expl_input_ids_2', 'expl_attention_mask_2', 'expl_input_ids_3', 'expl_attention_mask_3', 'expl_input_ids_4', 'expl_attention_mask_4'])].to_dict('records')
        expl_features_1 = features_df.loc[:, ~features_df.columns.isin(['labels', 'input_ids', 'attention_mask', 'aux_labels_2', 'expl_input_ids_2', 'expl_attention_mask_2', 'aux_labels_3', 'expl_input_ids_3', 'expl_attention_mask_3', 'aux_labels_4', 'expl_input_ids_4', 'expl_attention_mask_4'])].rename(
            columns={'aux_labels_1': 'labels', 'expl_input_ids_1': 'input_ids', 'expl_attention_mask_1': 'attention_mask'}).to_dict('records')

        expl_features_2 = features_df.loc[:, ~features_df.columns.isin(['labels', 'input_ids', 'attention_mask', 'aux_labels_1', 'expl_input_ids_1', 'expl_attention_mask_1', 'aux_labels_3', 'expl_input_ids_3', 'expl_attention_mask_3', 'aux_labels_4', 'expl_input_ids_4', 'expl_attention_mask_4'])].rename(
        ...
        expl_features_4 = super().__call__(expl_features_4, return_tensors)
...
class TaskPrefixTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        expl_outputs_4 = model(**inputs['expl_4'])

        loss = self.alpha * pred_outputs.loss + (1. - self.alpha) * (expl_outputs_1.loss + expl_outputs_2.loss + expl_outputs_3.loss + expl_outputs_4.loss) / 4.
        return (loss, {'pred': pred_outputs, 'expl_1': expl_outputs_1, 'expl_2': expl_outputs_2, 'expl_3': expl_outputs_3, 'expl_4': expl_outputs_4}) if return_outputs else loss
    ...
    def prediction_step(
        ...
        if self.output_rationale:
            ...
            expl_outputs_4 = super().prediction_step(model, inputs['expl_4'], prediction_loss_only=False, ignore_keys=ignore_keys)
        else:
